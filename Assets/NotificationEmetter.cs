...
public class NotificationEmetter : MonoBehaviour
{
    ...
    IEnumerator SendNotifROutine(string topic,NotifInfo info)
    {
        ...
        {
            // to = "/topics/" + topic,
            condition = "'" + "otherr" + "' in topics || '" + topic + "' in topics",
            priority = "high",
    ...
    public void SendMessageToTopic2(string topic,NotifInfo info)
    {
        ...
        tRequest.Method = "post";
        //serverKey - Key from Firebase cloud messaging server  
        tRequest.Headers.Add(string.Format("Authorization: key={0}", "AAAAJTkHlQ8:APA91bEsHMzYP8KA9ZD2jK2qr47cTUX53rb5qy-IVgEKjqdj3AjS43V8Oshb8ix_ZZez4TOBaAY-LcKxfUJA4bE4s-9abkvO00YCur5d0BtS64r4uAQ5QtnBarkJ4owgtVEmZ4IGNSoK"));
        //Sender Id - From firebase project setting  
        tRequest.Headers.Add(string.Format("Sender: id={0}", "159870588175"));
        tRequest.ContentType = "application/json";
        var payload = new
        {
            to = "/topics/" + topic,
