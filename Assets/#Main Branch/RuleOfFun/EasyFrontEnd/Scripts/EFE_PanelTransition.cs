...
public class EFE_PanelTransition : MonoBehaviour {
	...
	void Awake () 
	{
		...
		efe_base = efe_base_comp.gameObject;
	...
	public void DoTransitionIn()
	{
		...
		if (efe_base_comp.currentPanel)
		if (efe_base_comp.currentPanel != gameObject)
		if(efe_base_comp.currentPanel.GetComponent<EFE_PanelTransition>())
			efe_base_comp.currentPanel.GetComponent<EFE_PanelTransition>().DoTransitionOut();

		efe_base_comp.previousPanel = efe_base_comp.currentPanel;
		efe_base_comp.currentPanel = this;
	...
	public void DoTransitionOut()
	{
		...
		gameObject.SetActive(true);
		float transitionSpeed = fadeSpeed;
		if (efe_base_comp)
		if (efe_base_comp.currentPanel)
		if (efe_base_comp.currentPanel.GetComponent<EFE_PanelTransition>())
					transitionSpeed = efe_base_comp.currentPanel.GetComponent<EFE_PanelTransition>().transitionSpeed;
	...
	*/
	...
	public void TransitionComplete()
	{
		...
		//efe_base_comp.previousPanel.gameObject.SetActive(false);
	...
	public void  SwitchPanelsAndFadeOut()
	{
		...
		//switch panels
		efe_base.GetComponent<EFE_Base>().currentPanel.gameObject.active=true;
		//now fade out
