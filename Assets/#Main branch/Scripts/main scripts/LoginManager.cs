...
using UnityEngine.SocialPlatforms.GameCenter;
    ...
    public class LoginManager : MonoBehaviour
    {
       ...
       public bool UIEnabled = true;
        ...
        public void ShowToast(string message="Hello, Great Work!!!")
        {
        
                NativeControl.Instance().ShowToast(message, ToastLenght.LENGTH_LONG);
        }
        public TMPro.TMP_InputField emailF, passwordF, displayNameF, phoneNumberF, receivedCodeF;
        public TMPro.TMP_InputField emailLogin, passwordLogin;
    ...
    public void Login()
        ...
        public void SignByPhone()
        {
            GetVariablesFromInputField();
            VerifyPhoneNumber();
        }
        public void VerifyReceivedPhoneCodeFunction()
        {
            GetVariablesFromInputField();
        ...
        public void DebugLog(string s,bool showTaost=false)
        {
            Debug.Log(s);
            logText += s + "\n";
       ...
       public IEnumerator SaveTocken(string token)
        {
            while (UserManager.instance==null) yield return new WaitForSeconds(1);
            while (UserManager.instance.user==null) yield return new WaitForSeconds(1);
            while (UserManager.instance.user.id.Length<7) yield return new WaitForSeconds(1);
          //  var fcmToken = FirebaseInstanceId.Instance.Token;
            UserManager.instance.user.token =token;
            UserManager.instance.SaveUser();
        }
        ...
        protected bool LogTaskCompletion(Task task, string operation)
        {
            ...
            if (task.IsCanceled)
            {
                DebugLog(operation + " canceled.",true);
            }
            else if (task.IsFaulted)
            {
                DebugLog(operation + " encounted an error.",true);
                foreach (Exception exception in task.Exception.Flatten().InnerExceptions)
                {
                    ...
                    DebugLog(authErrorCode + exception.ToString(),true);
                }
            }
            else if (task.IsCompleted)
            {
                DebugLog(operation + " completed",true);
                complete = true;
            }
            return complete;
        }
        ...
        public Task SigninWithEmailAsync()
        {
            ...
            if (signInAndFetchProfile)
            {
                return auth.SignInAndRetrieveDataWithCredentialAsync(
                  Firebase.Auth.EmailAuthProvider.GetCredential(emailLogin.text, passwordLogin.text)).ContinueWithOnMainThread(
                    HandleSignInWithSignInResult);
            }
            else
            {
                return auth.SignInWithEmailAndPasswordAsync(emailLogin.text, passwordLogin.text)
        ...
        void HandleSignInWithUser(Task<Firebase.Auth.FirebaseUser> task)
        {
            ...
            if (LogTaskCompletion(task, "Sign-in"))
            {
                ...
                DebugLog(String.Format("Hello {0} !!", task.Result.DisplayName),true);
                OnSucessLogin();
            }
        }
        ...
        protected void SendPasswordResetEmail()
        {
            auth.SendPasswordResetEmailAsync(email).ContinueWithOnMainThread((authTask) => {
                ...
                {
                    DebugLog("Password reset email sent to " + email,true);
        ...
        protected void VerifyPhoneNumber()
        {
            ...
            phoneAuthProvider.VerifyPhoneNumber(phoneNumber, phoneAuthTimeoutMs, null,
              ...
              verificationFailed: (error) => {
                  ...
                  DebugLog("Phone Auth, verification failed" , true);
              },
              codeSent: (id, token) => {
                  phoneAuthVerificationId = id;
                  DebugLog("Phone Auth, code sent",true);
              },
              codeAutoRetrievalTimeOut: (id) => {
                  DebugLog("Phone Auth, auto-verification timed out",true);
              });
        }
        ...
        void GetVariablesFromInputField()
        {   
            email = emailF.text;
            password = passwordF.text;
            displayName = displayNameF.text;
            phoneNumber = phoneNumberF.text;
            receivedCode = receivedCodeF.text;
        }
